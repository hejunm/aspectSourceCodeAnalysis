...
//trackedClass hook的selector的集合。
@property (nonatomic, strong) NSMutableSet *selectorNames;
//记录其子类hook selector信息。
...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            ...
            /** 通过objc_getAssociatedObject 获取容器*/
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector);
            
            /** 将block封装到AspectIdentifier对象中，并将这个对象保存到aspectContainer容器中*/
...
/** block的参数列表要和被hook的selector的参数列表相匹配。 从index=2开始验证，从下面的代码中可以很容易的看出验证规则*/
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    /** 当hook一个对象的selector时，实现原理与KVO相似。 1，成一个子类；2，aspect_swizzleForwardInvocation*/
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    /**1, 不可以hook @"retain", @"release", @"autorelease", @"forwardInvocation:"*/
    ...
    /**2, 当hook dealloc时，切入点只能是AspectPositionBefore。*/
    // Additional checks.
    ...
    ///self是class object时，就要保证同一个类继承链中 selector只能被hook一次。
    // Search for the current class and the class hierarchy IF we are modifying a class object
    if (class_isMetaClass(object_getClass(self))) {
        Class klass = [self class];
        
        //swizzledClassesDict 是单例的字典，key:currentClass, value：currentClass对应的AspectTracker。
        NSMutableDictionary *swizzledClassesDict = aspect_getSwizzledClassesDict();
        Class currentClass = [self class];

        /** 判断currentClass 的子类是否hook过这个selector*/
        ...
        /** 判断currentClass 的父类是否hook过这个selector*/
        do {
        ...
        do {
            ...
            if (subclassTracker) { //将子类hook selector的信息添加到父类中
...
// 当子类hook selector后，将与子类对应的AspectTracker对象保存到superClass中。这种数据结构的设计就是为了实现 在一个类继承链中一个selector只能被hook一次
- (void)addSubclassTracker:(AspectTracker *)subclassTracker hookingSelectorName:(NSString *)selectorName {
...
// 在父类中删除某个子类的selectorName对应的AspectTracker对象。
- (void)removeSubclassTracker:(AspectTracker *)subclassTracker hookingSelectorName:(NSString *)selectorName {
    NSMutableSet *trackerSet = self.selectorNamesToSubclassTrackers[selectorName];
...
// 如果该类hook某个selector， 此selector已经被子类hook过，为了保证‘一个类继承链中一个selector只能被hook一次’ 此时要显示错误信息。 提示subClass1...已经hook过此selector。
- (NSSet *)subclassTrackersHookingSelectorName:(NSString *)selectorName {
...
// 从NSInvocation实例中获取每一个参数对应的值。这是直接从ReactiveCocoa 中得到的。
...
//self.methodSignature中的参数：  0：对象； 1：SEL;  其他： 对应selector中的参数列表。 所以在取值时从index=2开始
- (NSArray *)aspects_arguments {
...
//每进行一次hook操作， 就会生成一个对应的AspectIdentifier对象。 相当于将block分装起来。
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error {
    NSCParameterAssert(block);
    NSCParameterAssert(selector);
...
/**
 *  类+selector   ===>  对应一个AspectsContainer
 *  对象+selector  ====> 对应一个AspectsContainer
 *
 */
